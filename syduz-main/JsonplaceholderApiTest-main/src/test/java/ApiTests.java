...
public class ApiTests {
    ...
    public void searchEmail() {
        Response response = RestAssured.given()
                .queryParam("email", "Jayne_Kuhic@sydney.com")
                .get("https://jsonplaceholder.typicode.com/comments")
                .then()
                .statusCode(200)
                .extract().response();
        assertThat(response.getBody().jsonPath().getString("email"), Matchers.containsString("Jayne_Kuhic@sydney.com"));
        String data = response.asString();
        System.out.println("Response Data is " + data);
    }
    ...
    public void UpdateUserDetails() {
        ...
        // Step 3: Create a JSON request which contains all the fields which we wish to
        // update.
        JsonObject requestParams = new JsonObject();
        requestParams.addProperty("userId", 8);
        // Meanwhile, we have already 10 users which have userId=8 and now we'll add
        // 11th user
        ...
        // Here, we capture the response for PUT request by passing the associated idD
        // in the baseURI
